"""Add encrypted_key to ProductAPIKey

Revision ID: 028d183da5b9
Revises: 96902101434f
Create Date: 2025-09-21 19:26:39.059860
...
revision: str = '028d183da5b9'
down_revision: Union[str, None] = '96902101434f'
...
def upgrade() -> None:
    ...
    # Only add the encrypted_key column, don't drop event tables

    # Add the encrypted_key column as nullable first
    op.add_column('product_api_keys', sa.Column('encrypted_key', sa.Text(), nullable=True))

    # For existing records, set a placeholder encrypted value
    # This is a temporary solution - in production you'd need to handle this more carefully
    connection = op.get_bind()
    connection.execute(sa.text("UPDATE product_api_keys SET encrypted_key = 'LEGACY_KEY_NOT_RECOVERABLE' WHERE encrypted_key IS NULL"))

    # Now make the column NOT NULL
    op.alter_column('product_api_keys', 'encrypted_key', nullable=False)
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('product_api_keys', 'encrypted_key')
