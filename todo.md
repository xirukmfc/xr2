2. Лимиты по месяцам или дням? Наверное лучше в месяц?
3. Test it all (manual)
2. Автотеты
3. Нужно ли в итоге сейчас подключать оплату или нет? И нужны доки политика и тд
2. Инструкция по использованию 
3. Нужно где описать какие SDK есть

1. Выверить используемые модели в test AI + то что считается токенах. И ккак обновлять это на регуляной основе
2. Set up monitoring system. Новые пользователи, подписки, использование, производитлеьностЬ, нагрузка, аткак
3. Для прода оставить только авторизация через Gmail/Demo пользователь 
4. Проверка уязвимостей на проде
5. Сделать/обновить readme и навести порядок, отключить лишние логи 
...
0. Проверить ссылки, завести почты и контакты на лэнжинге
1. Запуск проекта
