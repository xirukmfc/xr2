...
// Use from Apex (via callout through named credentials / remote site) or LWC via server-side proxy
export async function getPrompt(conn: XR2Connection, params: GetPromptParams) {
    const url = `${BASE_URL.replace(/\/$/, '')}/api/v1/get-prompt`;
    const response = await axios.post(url, { ...params, source_name: 'salesforce_sdk' }, {
        headers: {
