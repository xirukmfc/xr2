...
### Added
- **Variable Values** field in Get Prompt operation — replace `{{variable}}` placeholders directly in the node without a separate Code node
- Support for n8n expressions in variable values (e.g. `{{ $json.customer_name }}`)
- Automatic default value fallback from prompt variable definitions
- `variables_used` field in output when variables are rendered
