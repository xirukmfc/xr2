#!/bin/bash

# Скрипт для деплоя всех исправлений API на продакшн сервер
# Использование: ./deploy-api-fixes.sh

set -e

GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
BLUE='\033[0;34m'
NC='\033[0m'

# Хост берётся из .deploy.env (в .gitignore) или из окружения
[ -f "$(dirname "$0")/.deploy.env" ] && . "$(dirname "$0")/.deploy.env"
REMOTE_HOST="${XR2_PROD_HOST:?не задан XR2_PROD_HOST — создайте .deploy.env по образцу .deploy.env.example}"
REMOTE_PATH="/opt/xr2"
LOCAL_PATH="$(pwd)"

echo -e "${BLUE}🚀 Деплой всех исправлений API на продакшн${NC}"
echo "=============================================="
echo ""
echo -e "${YELLOW}📋 Изменения:${NC}"
echo "  ✅ prompt-editor/components/test-modal.tsx - использует apiClient.request()"
echo "  ✅ prompt-editor/components/analytics/ROIAnalysis.tsx - использует apiClient.request()"
echo "  ✅ prompt-editor/components/analytics/ABTestManager.tsx - исправлены пути на /ab-tests-simple/test"
echo "  ✅ prompt-editor/lib/api.ts - исправлен тип метода request()"
echo "  ✅ nginx/nginx.prod.conf - добавлено правило для /api/tokenize/"
echo ""

# Копирование измененных файлов на сервер
echo -e "${YELLOW}📦 Копирование файлов на сервер...${NC}"
scp prompt-editor/components/test-modal.tsx "${REMOTE_HOST}:${REMOTE_PATH}/prompt-editor/components/test-modal.tsx"
scp prompt-editor/components/analytics/ROIAnalysis.tsx "${REMOTE_HOST}:${REMOTE_PATH}/prompt-editor/components/analytics/ROIAnalysis.tsx"
scp prompt-editor/components/analytics/ABTestManager.tsx "${REMOTE_HOST}:${REMOTE_PATH}/prompt-editor/components/analytics/ABTestManager.tsx"
scp prompt-editor/lib/api.ts "${REMOTE_HOST}:${REMOTE_PATH}/prompt-editor/lib/api.ts"
scp nginx/nginx.prod.conf "${REMOTE_HOST}:${REMOTE_PATH}/nginx/nginx.prod.conf"

echo -e "${GREEN}✅ Файлы скопированы${NC}"

# Выполнение обновления на сервере
echo -e "${YELLOW}🔨 Выполнение обновления на сервере...${NC}"
ssh "${REMOTE_HOST}" << 'ENDSSH'
    set -e
    cd /opt/xr2
    
    echo "📋 Проверка файлов..."
    ls -la prompt-editor/components/test-modal.tsx
    ls -la prompt-editor/components/analytics/ROIAnalysis.tsx
    ls -la prompt-editor/components/analytics/ABTestManager.tsx
    ls -la prompt-editor/lib/api.ts
    ls -la nginx/nginx.prod.conf
    
    echo "🔨 Пересборка фронтенда..."
    docker compose --env-file .env.prod -f docker-compose.prod.yml build frontend
    
    echo "🔄 Перезапуск nginx..."
    docker compose --env-file .env.prod -f docker-compose.prod.yml restart nginx
    
    echo "🔄 Перезапуск фронтенда..."
    docker compose --env-file .env.prod -f docker-compose.prod.yml up -d frontend
    
    echo "⏳ Ожидание запуска сервисов..."
    sleep 15
    
    echo "📊 Статус сервисов:"
    docker compose --env-file .env.prod -f docker-compose.prod.yml ps frontend nginx
    
    echo ""
    echo "✅ Обновление завершено!"
ENDSSH

echo ""
echo -e "${GREEN}🎉 Деплой завершен успешно!${NC}"
echo ""
echo -e "${YELLOW}🧪 Проверьте работу:${NC}"
echo "  1. Test with AI: https://xr2.uk/editor/[prompt-id]"
echo "     - Должен работать запрос на /internal/llm/test-run"
echo "  2. Tokenize: проверьте подсчет токенов в редакторе"
echo "     - Должен работать запрос на /api/tokenize/precise"
echo "  3. ROI Analysis: проверьте аналитику"
echo "     - Должен работать запрос на /internal/analytics/roi"
echo "  4. A/B Tests: проверьте страницу /analytics/ab-tests"
echo "     - Должны работать запросы на /internal/ab-tests-simple/test"
echo ""
echo -e "${YELLOW}📋 Полезные команды для отладки:${NC}"
echo "  docker logs xr2_frontend_prod --tail 50 -f"
echo "  docker logs xr2_nginx_prod --tail 50 -f"
echo "  docker logs xr2_app_prod --tail 50 -f"
echo ""
